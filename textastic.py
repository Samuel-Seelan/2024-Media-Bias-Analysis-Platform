...
class Textastic:
    ...
    @staticmethod
    def clean_text(text):
        """Preprocess text: lowercase, remove punctuation, and whitespace."""
    ...
    def load_text(self, filename, label=None, parser=None):
        ...
        if parser is None:
            parser = self._default_parser  # Note the change here to use the instance method
        try:
    ...
    def compare_num_words(self):
        """A simplistic visualization comparing the number of words in each text file."""
        labels = list(self.data['numwords'].keys())
        values = [self.data['numwords'][label] for label in labels]
